...
public class TLKPrivateKey {
    ...
    public TLKPrivateKey(){
        this( CurveParam.SECP256_K1);
    }

    public TLKPrivateKey(int curveParamType){
        mCurveParam = EcTools.getCurveParam(curveParamType);

        mPrivateKey = getOrCreatePrivKeyBigInteger( null );
        mPublicKey = new TLKPublicKey(findPubKey( mPrivateKey ), mCurveParam);
    }

    public TLKPrivateKey(String base58Str ) {

        String[] split = TLKEcUtil.safeSplitEosCryptoString( base58Str );
        byte[] keyBytes;

        if ( split.length == 1 ){
            mCurveParam = EcTools.getCurveParam( CurveParam.SECP256_K1);
            keyBytes = TLKEcUtil.getBytesIfMatchedSha256( base58Str, null);
        }
        else {
            if ( split.length < 3 ) {
            ...
            mCurveParam = TLKEcUtil.getCurveParamFrom( split[1]);
            keyBytes = TLKEcUtil.getBytesIfMatchedRipemd160( split[2], split[1], null);
        }
    ...
    public TLKPublicKey getPublicKey() {
        return mPublicKey;
    }
    ...
    public String toString() {
        ...
        return TLKEcUtil.encodeEosCrypto( PREFIX, mCurveParam , getBytes());
    }
