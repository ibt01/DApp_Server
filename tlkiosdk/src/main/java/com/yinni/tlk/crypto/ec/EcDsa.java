...
import com.yinni.tlk.crypto.Hmac;
...
public class EcDsa {
    ...
    private static BigInteger deterministicGenerateK(CurveParam curveParam, byte[] hash, BigInteger d, SigChecker checker, int nonce ){
        ...
        // Step d
        TLKByteWriter bwD = new TLKByteWriter(32 + 1 + 32 + 32);
        bwD.putBytes(v);
        bwD.put((byte) 0x00 );
        bwD.putBytes( dBytes );
        bwD.putBytes(hash);
        ...
        // Step f
        TLKByteWriter bwF = new TLKByteWriter(32 + 1 + 32 + 32);
        bwF.putBytes(v);
        bwF.put((byte) 0x01 );
        bwF.putBytes(dBytes);
        bwF.putBytes(hash);
        ...
        while ((t.signum() <= 0) || (t.compareTo( curveParam.n()) >= 0) || !checker.checkSignature(curveParam, t)) {
            TLKByteWriter bwH = new TLKByteWriter(32 + 1);
            bwH.putBytes(v);
            bwH.put((byte) 0x00);
    ...
    public static EcSignature sign(Sha256 hash, TLKPrivateKey key ) {
        ...
        TLKPublicKey pubKey = key.getPublicKey();

        for (int i = 0; i < 4; i++) {
            TLKPublicKey recovered = recoverPubKey(curveParam, data, signature, i);
    ...
    public static TLKPublicKey recoverPubKey(byte[] messageSigned, EcSignature signature ) {
        return recoverPubKey( signature.curveParam, messageSigned, signature, signature.recId);
    }

    public static TLKPublicKey recoverPubKey(CurveParam curveParam, byte[] messageSigned, EcSignature signature, int recId ) {
        ...
        return new TLKPublicKey( q.getEncoded() );
    }
