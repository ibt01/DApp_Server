...
public class LoginFilter implements Filter {
    ...
    static {
          setUrlReleaseRestriction.add("/getDapps");
//        setUrlReleaseRestriction.add("/saveUserInfo");
    }
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        //response.setHeader("Access-Control-Allow-Headers", "my_privatekeypass,x-auth-token,Origin,Access-Token,X-Requested-With,Content-Type, Accept,multipart/form-data,Authorization,server_pwd,server_id,version,system_name,system_version,pwd,lang,");
        ...
        String server_pwd = ((HttpServletRequest) servletRequest).getHeader("server_pwd");
        String server_id = ((HttpServletRequest) servletRequest).getHeader("server_id");
        ...
        if (!StringUtils.isEmpty(server_pwd)) {
            Integer checkStatus = checkCookieAndPrivilege(server_pwd, server_id, priaveKeyPassManager, url);
            if (checkok == checkStatus) {
                userClient.setCookie(server_pwd);
                userClient.setUserID(server_id);
    ...
    Integer checkCookieAndPrivilege(String cookie, String uid,String priaveKeyPassManager, String urlOuter) {
        if (!"serverid1a80eccc-352e-4940-8df0-7a437e972a4a".equals(uid)){
            return checkcookieinvalid;
        }

        if (!"9e8dd5c8-25e3-41f1-999b-8742896894a2".equals(cookie)){
            return checkcookieinvalid;
        }
        return checkok;
    }
